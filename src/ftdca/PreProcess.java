...
public class PreProcess extends javax.swing.JFrame {
            ...
            .addGroup(layout.createSequentialGroup()
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(layout.createSequentialGroup()
                        ...
                        .addGap(18, 18, 18)
                        .addComponent(SaveButton, javax.swing.GroupLayout.DEFAULT_SIZE, 158, Short.MAX_VALUE))
                    .addGroup(layout.createSequentialGroup()
                        .addGap(21, 21, 21)
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
    ...
    private void ChooseFileButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ChooseFileButtonActionPerformed
        ...
        try {
            ...
            while (iterator.hasNext()) { //while json array is not empty
                row.addRow(new Object[]{i++, iterator.next(), iterator.next(), iterator.next()}); //adding a new row into table
            }
    ...
    private void AddRowButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_AddRowButtonActionPerformed
        if(IsTableLoaded == true){ //if the table alredy filled with data then proceed
            if(NewRowField.getText().isEmpty()){ //if nothing is typed but add button is clicked
                JOptionPane.showMessageDialog(null, "Please enter something!"); //showing this
            } else { 
    ...
    private void DeleteRowButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_DeleteRowButtonActionPerformed
        if(IsTableLoaded == true){ //if the table is loaded with data
            String no = DeleteRowNum.getText(); //get the id no
            if(no.isEmpty()){ //if nothing is given
                JOptionPane.showMessageDialog(null, "Please enter an ID number!"); //show this message
            } else {
                int Del = Integer.parseInt(DeleteRowNum.getText()); //type cast string into int
                int r = row.getRowCount(); //get the total number of the row present in the table

                for(int j = 0; j < r; j++){ //loop throught the rows
                    if(Del == (int)row.getValueAt(j, 0)){ //if the id and the the no provided by the user matches
                        Del = j; //this is the true row number, there may be id no 100 but total row 90, so in that case, we need true row number to delete the row correctly
                        break;
                    }
                }
                if(Del >= r || Del < 0){ //if the number exceeds total row number of rows or negative number
                    JOptionPane.showMessageDialog(null, "Row "+Del+" Does Not Exist");
                } else{
                    row.removeRow(Del); //delete the row
                    DeleteRowNum.setText(""); //clear the row no
                }
            }
        } else {
    ...
    private void ClearButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ClearButtonActionPerformed
        if(IsTableLoaded){
            try{
                while(row.getRowCount() != 0){  //when there is nonzero number of rows on the table
                    row.removeRow(row.getRowCount() - 1); //remove the last row
                }
                i = 1;
                IsTableLoaded = false; //there is no data so, flag is false
            } catch(Exception e){
                e.printStackTrace();
            }
        }
